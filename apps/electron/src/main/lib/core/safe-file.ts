...
import { chmodSync, writeFileSync, renameSync, existsSync, copyFileSync, readFileSync, unlinkSync } from 'node:fs'
...
export function writeTextFileAtomic(filePath: string, content: string, mode = 0o600): void {
  const tmpPath = filePath + '.tmp'
